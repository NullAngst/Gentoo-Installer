#!/usr/bin/env bash
# =============================================================================
#  gentoo-install.sh - a guided, verbose Gentoo Linux installer for amd64
# =============================================================================
#
#  What it does
#  ------------
...
show_hardware() {
    ...
    printf '  %-22s %s\n' "CPU:" "${CPU_MODEL:-unknown} (${CPU_VENDOR:-unknown})"
    printf '  %-22s %s\n' "CPU threads:" "$NPROC"
    printf '  %-22s %s\n' "Memory:" "about ${RAM_GIB} GiB"
    printf '  %-22s %s\n' "x86-64-v3 capable:" "$CPU_X86_64_V3 (AVX2 generation or newer)"
    printf '  %-22s %s\n' "Firmware boot mode:" "$( [[ $BOOT_MODE == uefi ]] && echo "UEFI (${UEFI_BITS:-64}-bit)" || echo "Legacy BIOS" )"
    if [[ $BOOT_MODE == "uefi" ]]; then
        printf '  %-22s %s\n' "Secure Boot:" "$SECURE_BOOT"
    fi
    printf '  %-22s %s\n' "Virtual machine:" "$( [[ $VIRT == none ]] && echo "no (real hardware)" || echo "yes ($VIRT)" )"
    printf '  %-22s %s\n' "Laptop:" "$IS_LAPTOP"
    printf '  %-22s %s\n' "Wi-Fi adapter:" "$HAS_WIFI"
    printf '  %-22s %s\n' "Bluetooth adapter:" "$HAS_BT"
    local g
    if (( ${#GPU_NAMES[@]} == 0 )); then
        printf '  %-22s %s\n' "Graphics:" "none detected"
    else
...
q_system_type() {
    ...
    choose DE "Desktop environment" "plasma" \
    ...
    if [[ $DE == "sway" ]]; then
        ...
        ask_yn SWAY_AUTOSTART "Start Sway automatically after logging in on tty1?" y
    elif [[ $DE != "none" ]]; then
        local def_dm
        ...
        if yesno "Install the graphical login screen (${def_dm})?" y; then DM=$def_dm; fi
    fi
    ...
    ask_yn USE_BINPKG "Use Gentoo's official binary packages?" y
    BINHOST_V3="no"
    ...
    ask_yn TUNE_CPU_FLAGS "Detect and use this CPU's exact instruction set flags?" y
    ...
    choose KERNEL_PKG "Kernel" "gentoo-kernel-bin" \
...
q_disk() {
    ...
    choose DISK "Target disk" "${opts[0]%%|*}" "${opts[@]}"

    echo
    info "Current contents of ${DISK}:"
    lsblk -po NAME,SIZE,FSTYPE,LABEL,MOUNTPOINT "$DISK" | sed 's/^/    /'
    echo
...
q_part_mode() {
    ...
    choose PART_MODE "Partitioning" "auto" \
...
q_filesystem() {
    ...
    choose FS "Root filesystem" "ext4" "${opts[@]}"
}
...
q_encryption() {
    ...
    ask_yn ENCRYPT "Encrypt the root partition?" n
    if [[ $ENCRYPT == "yes" ]]; then
...
q_swap() {
    ...
    opts+=("none|No swap")
    choose SWAP_MODE "Swap" "zram" "${opts[@]}"
    SWAP_SIZE_GIB=""
    if [[ $SWAP_MODE == "partition" && $PART_MODE == "auto" ]]; then
        ask SWAP_SIZE_GIB "Swap partition size in GiB" "$ram_cap" valid_positive_int
    fi
}
...
q_bootloader() {
    ...
    choose BOOTLOADER "Bootloader" "grub" \
...
q_manual_partitions() {
    ...
    echo
    lsblk -po NAME,SIZE,FSTYPE,PARTTYPENAME,LABEL,MOUNTPOINT 2>/dev/null | sed 's/^/    /'
    echo

    local used="" fstype bytes
    ...
    if [[ $BOOT_MODE == "uefi" ]]; then
        ...
        if [[ $fstype == "vfat" ]]; then
            ...
            ask_yn FORMAT_ESP "Format ${ESP_PART}? (answer no to keep the existing boot files)" n
        else
...
show_auto_layout() {
    ...
    echo
    info "Planned layout for ${DISK} (GPT partition table):"
    if [[ $BOOT_MODE == "uefi" ]]; then
        printf '    %s   %-9s %s\n' "$(part_dev "$DISK" $n)" "1 GiB" "EFI system partition, FAT32, mounted at /efi"
    else
        printf '    %s   %-9s %s\n' "$(part_dev "$DISK" $n)" "1 MiB" "BIOS boot partition (used by GRUB, no filesystem)"
    fi
    n=$(( n + 1 ))
    if [[ $NEED_BOOT_PART == "yes" ]]; then
        printf '    %s   %-9s %s\n' "$(part_dev "$DISK" $n)" "1 GiB" "boot partition, ext4, mounted at /boot"
        n=$(( n + 1 ))
    fi
    if [[ $SWAP_MODE == "partition" ]]; then
        printf '    %s   %-9s %s\n' "$(part_dev "$DISK" $n)" "${SWAP_SIZE_GIB} GiB" "swap"
        n=$(( n + 1 ))
    fi
    printf '    %s   %-9s %s\n' "$(part_dev "$DISK" $n)" "the rest" "root filesystem: ${fsdesc}"
    echo
    DUAL_BOOT="no"
    FORMAT_ESP="yes"
    GRUB_DISK="$DISK"
...
q_region() {
    ...
    ask NEW_HOSTNAME "Hostname" "gentoo" valid_hostname
...
q_accounts() {
    ...
    choose PRIV_TOOL "Administrator tool" "sudo" \
...
q_hardware_network() {
    ...
    if [[ $HAS_NVIDIA == "yes" ]]; then
        ...
        choose GPU_DRIVER "NVIDIA driver" "nvidia" \
    ...
    choose NET_TOOL "Network manager" "$def_net" "${nopts[@]}"
    ...
    ask_yn WANT_BT "Install Bluetooth support?" "$def_bt"

    WANT_CUPS="no"
    if [[ $DE != "none" ]]; then
        say "CUPS is the printing system. Only needed if you print from this computer."
        ask_yn WANT_CUPS "Install printer support (CUPS)?" n
    fi
    ...
    ask_yn WANT_SSH "Enable the SSH server?" "$def_ssh"
}
...
q_software() {
    ...
    local mirror_mode
    choose mirror_mode "Download mirror" "cdn" "${mopts[@]}"
    GENTOO_MIRRORS_VALUE=""
    DIST_BASE="https://distfiles.gentoo.org"
    if [[ $mirror_mode == "select" ]]; then
    ...
    if [[ $DE != "none" ]]; then
        ...
        ask_yn WANT_FLATPAK "Install Flatpak and add the Flathub repository?" y
